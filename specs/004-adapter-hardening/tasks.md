...
- [X] T001 [P] Create `.specify/workflows/runs/004-adapter-hardening.md` with frontmatter for orchestration and review state
- [X] T002 [P] Capture the current success-path output as the FR-005 reference: run `.specify/adapters/claude-code/project.sh` against a temp fixture, store the result under the system temp directory, and record its checksum in `.specify/workflows/runs/004-adapter-hardening.md`
...
- [X] T003 Add `retain: 3` under the `backup` key in `.specify/adapters/claude-code/adapter.yml` (see FR-003 for why pruning is subordinate), and CORRECT the existing `backup.pattern` to describe the filename the script actually produces — it gained a `%PID%` disambiguator that the declaration never recorded, so the declared pattern currently describes a file that is never written (FR-001)
...
- [X] T004 [US1] Write failing cases for a `retention` group in `.specify/adapters/claude-code/tests/run-tests.sh`: below the limit adds without removing; at the limit removes the oldest and holds steady; **a fixture pre-seeded with backups the adapter did not create** is pruned too, since the policy applies to what it finds rather than only to what it made; all three pruning-failure modes leave the projection successful; files not matching the declared pattern are never considered; the live target is never a pruning candidate. Run and watch them fail (FR-001, FR-002, FR-003, SC-002)
- [X] T005 [US1] Implement pruning in `.specify/adapters/claude-code/project.sh`, running AFTER a successful write and never before — pruning first would delete the safety net the write might still need. DERIVE the match glob from `backup.pattern` in `adapter.yml` rather than from the hardcoded `BACKUP_SUFFIX` constant: the script's own header claims nothing is hardcoded, which is currently false, and matching by a constant the declaration does not describe violates Principle II. Remove oldest first, and swallow any failure with a warning rather than failing the projection (FR-001, FR-002, FR-003)
- [X] T006 [US1] Run `.specify/adapters/claude-code/tests/run-tests.sh retention` and confirm all cases pass (SC-001, SC-002)
...
- [X] T007 [US2] Write failing cases for a `composition` group in `.specify/adapters/claude-code/tests/run-tests.sh` covering all three injected modes: truncation that still carries valid markers and the correct version, empty output, and a compose step exiting non-zero (US2 scenario 1 — the unchecked `head`/`tail`/`sed`/`awk` chain is exactly this case). Each asserts rejection BEFORE the write and a byte-unchanged target. Run and watch them fail (FR-004, SC-003)
- [X] T008 [US2] Replace the line-count check in `.specify/adapters/claude-code/project.sh` with a fingerprint comparison: extract the region from the composed file and compare its `cksum` against the `cksum` of the same `$SRC_CONTENT` used to compose it — exact by construction, and not the structural proxy a line count would be. Add the injection hook as a single guarded branch reading `AIJEDI_TEST_INJECT`, inert unless that variable is set, so FR-005's byte-identity on the success path is unaffected (FR-004, FR-005)
- [X] T009 [US2] Run `.specify/adapters/claude-code/tests/run-tests.sh composition` and confirm all cases pass (SC-003)
...
- [X] T011 [US3] Implement the lint group in `.specify/adapters/claude-code/tests/run-tests.sh` with three distinct outcomes — pass, fail, skipped — and a `skip()` counter separate from `ok()`, because a skipped group counted as passing is the vacuous assertion the PR #5 review caught in `stat -f` (FR-006, FR-008)
- [X] T012 [US3] Run `.specify/adapters/claude-code/tests/run-tests.sh lint` on this machine, where the linter is absent, and confirm it reports SKIPPED rather than PASSED (SC-005)

---

## Phase 6: Regression & Parity

- [X] T013 Run the full suite `.specify/adapters/claude-code/tests/run-tests.sh` and confirm every pre-existing group still passes — hardening must not alter correct behavior
- [X] T014 Verify FR-005 against the T002 reference: project into a temp fixture and confirm the success-path output is byte-identical to the pre-change reference. Then confirm read-only that `~/.claude/CLAUDE.md` still reports `current` via `project.sh --check` (SC-004)
- [X] T015 Record the reviewed no-change decision for `README.md` in `.specify/workflows/runs/004-adapter-hardening.md`: retention and linting are not operator-visible benefits, so no README claim changes (Principle XII, Authoring Constraints)
- [X] T016 Confirm no committed file under `.specify/adapters/` contains an absolute home path, a username, or a credential-shaped string (Principle IX)
